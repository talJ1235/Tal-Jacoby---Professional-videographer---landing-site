...
// ── Archive btn (active tab → moves to history) ────────────────
function ArchiveBtn({ lead, onArchive }) {
  const [confirm, setConfirm] = useState(false);
  if (confirm) return (
    <span className="delete-confirm">
      <span className="delete-confirm__txt">להיסטוריה?</span>
      <button className="delete-confirm__yes" onClick={() => { onArchive(lead.id, { status: 'סגור' }); setConfirm(false); }} title="כן">✓</button>
      <button className="delete-confirm__no"  onClick={() => setConfirm(false)} title="ביטול">✕</button>
    </span>
  );
  return (
    <button className="action-archive" onClick={() => setConfirm(true)} title="העבר להיסטוריה">
      {/* inbox/archive icon */}
...
// ── Delete btn (history tab → permanent soft-delete) ──────────
function DeleteBtn({ lead, onDelete }) {
  const [confirm, setConfirm] = useState(false);
  if (confirm) return (
    <span className="delete-confirm">
      <span className="delete-confirm__txt">מחק?</span>
      <button className="delete-confirm__yes" onClick={() => onDelete(lead.id)} title="כן, מחק">✓</button>
      <button className="delete-confirm__no"  onClick={() => setConfirm(false)} title="ביטול">✕</button>
    </span>
  );
  return (
    <button className="action-delete" onClick={() => setConfirm(true)} title="מחק לצמיתות">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export function Admin() {
  ...
  const [expandedMsg, setExpandedMsg]   = useState(null); // lead id with open message
                    ...
                    <>
                      <tr key={lead.id}>
                        ...
                        <td className="td-actions">
                          ...
                          {/* Message button — only when there's a message */}
                          {lead.message && (
                            <button
                          ...
                          {/* Archive (active tab) or Delete (history tab) */}
                          {activeTab === 'active'
                            ? <ArchiveBtn lead={lead} onArchive={patchLead} />
                            : <DeleteBtn  lead={lead} onDelete={removeLead} />
                          }
                        </td>
                      </tr>
                      {/* Expanded message row */}
                      {expandedMsg === lead.id && (
                        <tr key={`msg-${lead.id}`} className="msg-row">
                          <td colSpan={8}>
