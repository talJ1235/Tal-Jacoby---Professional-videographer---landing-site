...
// Normalize DB status: legacy 'new' → 'חדש'
...
function StatsBar({ leads }) {
  ...
  const newCount    = leads.filter((l) => normStatus(l.status) === 'חדש').length;
  const inProgress  = leads.filter((l) => normStatus(l.status) === 'בטיפול').length;
  const closed      = leads.filter((l) => normStatus(l.status) === 'סגור').length;
  return (
...
function NoteCell({ lead, onSave }) {
  ...
  return editing ? (
    <input
      ...
      onKeyDown={(e) => {
        if (e.key === 'Enter') save();
        if (e.key === 'Escape') { setVal(lead.notes || ''); setEditing(false); }
      }}
    />
  ) : (
...
function DeleteBtn({ lead, onDelete }) {
  ...
  return (
    <button className="action-delete" onClick={() => setConfirm(true)} title="מחק">
...
export function Admin() {
  ...
  // Normalize all lead statuses before rendering
